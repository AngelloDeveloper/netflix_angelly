<?php
  ...
  //inatancias de objetos
  $objConexion = new Conexion();
  $conexion = $objConexion->getConexion();
  $banco = new Banco();
  $arrBancos = $banco->getBancos();
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h3 mb-0 text-gray-800">Página de Transferencias</h1>    
                    <a href="banca.php" class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm">
                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-danger">Registrar Transferencias</h6>
                            </div>
                            <div class="card-body">
                                <form id="formRef">
                                    <input type="text" class="form-control mb-3" name="ref" id="ref" placeholder="Referencia" required pattern="[0-9]{3,3}" title="Ingresar 3 digitos">
                                    <select class="form-control mb-3" name="select" id="select" required>
                                        <option value="0">Seleccionar</option>
                                        <?php foreach ($arrBancos as $banco) { ?>
                                            <option value="<?= $banco['idBan'].'/'.$banco['codigo'] ?>">
                                                <?= $banco['typBan'] ?>
                                            </option>
                                        <?php } ?>
                                    </select>
                                    <input type="datetime-local" class="form-control mb-3" name="date" placeholder="Fecha y Hora" required>
                                    <input type="text"  class="form-control mb-3" name="monto" id="monto" placeholder="Monto" step="any" required pattern="[0-9]{2.01,30.01}">                                   
                    ...
                    <div class="col-md-8">
                        ...
                        <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-danger">Transferencias Registradas</h6>
                            </div>
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="tablax" class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                        <thead>
                                            <tr>
                                                <th>Transferencia</th>
                                                <th>Banco</th>
                                        ...
                                        <tbody>
                                            <?php  
                                                ...
                                                $result = mysqli_query($conexion, "SELECT ref.refCod, ref.refCompuesto,ref.dateRef, ref.monRef,ref.status, ban.typBan FROM referencia ref 
                                                INNER join banco ban
                                                ON ref.idBan = ban.idBan 
                                                WHERE ref.monRef > 0 and ref.status = 1
                                                ORDER BY dateRef DESC");                      
                                                 while($mostrar = mysqli_fetch_array($result))
                                                {
                                            ?>
                                            <tr>
                                                <td><?php echo $mostrar['refCompuesto'];?></td>
                                                <td><?php echo $mostrar['typBan'];?></td>
                                                <td><?php echo $mostrar['dateRef'];?></td>
                                                <td><?php echo $mostrar['monRef'];?></td>
                                                <td></td>
                                                <td>
                                                    <a href="editRef.php?refCod=<?php echo $mostrar['refCod'];?>" class="btn btn-secondary"> <i class="fas fa-marker"></i></a>
                                                    
                                                    <a href="deleteRef.php?refCod=<?php echo $mostrar['refCod'];?>" class="btn btn-danger"> <i class="fas fa-trash-alt"></i></a>
                                                </td>
